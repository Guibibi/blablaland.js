...
request(
  {
    uri:
      'https://raw.githubusercontent.com/GregVido/blablaland.js/master/README.md',
  },
...
app.get('/params.xml', (req, res) => {
  res.send(
    `<params><scriptadr value="/scripts/"/><socket port="12301" host="${servername}"/></params>`
  );
});
...
app.get('/connexion', (req, res) => {
  if (req.query.validform == 'OK') {
    for (var id in database) {
      if (
        database[id].pseudo.toUpperCase() == req.query.con_pseudo.toUpperCase()
      ) {
        if (
          database[id].password.toUpperCase() ==
          req.query.con_password.toUpperCase()
        ) {
...
app.get('/signup', (req, res) => {
  ...
  if (
    req.query.validform == 'Inscription' &&
    req.query.con_pseudo &&
    req.query.con_password
  ) {
    var loc1 = false;
    if (req.query.con_pseudo.length < 3) {
      ...
      for (var id in database) {
        if (
          database[id].pseudo.toUpperCase() ==
          req.query.con_pseudo.toUpperCase()
        ) {
    ...
    if (req.session.message) {
      file = file.replace(
        '<!-- {{message}} -->',
        `<message>${req.session.message}</message><br>`
      );
  ...
  file = file.replace(
    `<div id="chat"></div>`,
    `<message><div style="width: 140px; height: 80px;">${sign}</div></message>`
  );
  res.send(file);
});
...
app.get('/', (req, res) => {
  ...
  if (req.session.username) {
    file = file.replace(
      '{{form}}',
      `<h1>${req.session.username}</h1><br><a href="/disconnect">Déconnexion</a>`
    );
